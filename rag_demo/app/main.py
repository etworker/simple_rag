...
# 日志同时输出到文件和控制台
LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "app.log")
...
# 缓存目录（可配置，默认 ~/.cache/simple_rag/）
CACHE_DIR = config_store.get("cache.base_dir", os.path.join(os.path.expanduser("~"), ".cache", "simple_rag"))
...
history_store = ChatHistoryStore(history_dir=os.path.join(DATA_DIR, "chat_history"))
...
_ws_handler.setFormatter(logging.Formatter("%(asctime)s | %(name)s | %(message)s", datefmt="%H:%M:%S"))
# 挂到 root logger
