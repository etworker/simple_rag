...
class PreClassifyResult:
    """预分类结果"""
    category: Optional[str]  # None = 不确定，交给 LLM
    confidence: float
    reason: str
...
# 管理信息上下文关键词
_MGMT_CONTEXT_WORDS = {
    '修订', '版次', '版本', '生效', '批准', '发布', '页码', '页号',
    '受控', '编号', '手册编号', '文件编号', '下载时间', '打印',
}

# 封面/有效页/修订记录特征
_COVER_PAGE_WORDS = {
    '受控状态', '版次', '手册编号', '文件编号', '发布日期', '下载时间',
    '打印', '有效', '修订日期', '修订次数', '批准人',
}
...
def annotate_fragments(diff_item) -> List[AnnotatedFragment]:
    ...
    for ftype, old_frag, new_frag in diff_item.diff_fragments:
        ...
        results.append(AnnotatedFragment(
...
def _classify_fragment(old_text: str, new_text: str, context: str) -> str:
    ...
    if any(w in context for w in _MGMT_CONTEXT_WORDS):
        ...
        if len(old_clean) + len(new_clean) < 30:
            if _DATE_PATTERN.search(old_clean + new_clean) or _VERSION_PATTERN.search(old_clean + new_clean):
                return 'mgmt_info'

    # 纯数字（无单位）——可能是章节号也可能是数量，标为 ambiguous_number
    if old_clean.replace('.', '').replace('-', '').isdigit() and \
       new_clean.replace('.', '').replace('-', '').isdigit():
...
def pre_classify(diff_item) -> PreClassifyResult:
    ...
    if mgmt_keyword_count >= 3:
        # 段落含大量管理关键词
        if annotated and all(a.annotation in ('date', 'version', 'section_ref', 'mgmt_info', 'ambiguous_number') for a in annotated):
            return PreClassifyResult(
                category='metadata',
                confidence=0.9,
