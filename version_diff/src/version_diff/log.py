"""version_diff 的 opt-in 日志配置，供 examples / 独立使用。
...
file 路径默认 ``./logs/version_diff.log``，可用环境变量 ``VERSION_DIFF_LOG_FILE`` 覆盖。
"""
...
def configure_logger(level: str = "INFO") -> None:
    ...
    - file 路径：环境变量 ``VERSION_DIFF_LOG_FILE``，缺省 ``./logs/version_diff.log``
    - 同时 ``logger.enable("version_diff")`` 打开本库日志（库默认通过 disable 关闭）
    """
    logger.remove()
