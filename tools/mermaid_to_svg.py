...
def classify(nid: str, label: str) -> str:
    s = (nid + " " + label).lower()
    if any(k in s for k in ["ec2", "app", "rag_demo", "vllm", "llm", "kserve", "gpu", "节点", "compute"]):
        return "purple"
    if any(k in s for k in ["opensearch", "aurora", "dynamodb", "s3", "redis", "faiss", "vdb", "hist",
                            "aur", "cache", "vector", "pgvector", "数据库", "向量", "store"]):
        return "teal"
    if any(k in s for k in ["keycloak", "secret", "auth", "lambda", "api", "cognito", "用户", "iam", "identity"]):
        return "amber"
    if any(k in s for k in ["alb", "nlb", "cloudwatch", "ecr", "waf", "xray", "监控", "负载", "gateway"]):
        return "blue"
    return "gray"
...
def layout(nodes, edges):
    ...
    for l in layers:
        layers[l].sort(key=lambda x: order.index(x))
...
def build_svg(text: str):
    ...
    out = [f'<svg viewBox="0 0 {width:.0f} {height:.0f}" width="100%" xmlns="http://www.w3.org/2000/svg" role="img">']
    out.append('<title>架构图</title>')
    out.append('<defs><marker id="arw" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" '
    ...
    for sg in subgraphs:
        ...
        out.append(f'<rect x="{bx:.0f}" y="{by:.0f}" width="{bw:.0f}" height="{bh:.0f}" rx="18" '
                   f'fill="#EEEDFE" fill-opacity="0.16" stroke="#534AB7" stroke-width="1" stroke-dasharray="6 4"/>')
        out.append(f'<text x="{bx + 12:.0f}" y="{by + 16:.0f}" style="font:500 12px sans-serif; fill:#534AB7">{esc(sg["label"])}</text>')
    ...
    for s, d, elabel in edges:
        ...
        out.append(f'<path d="{path}" fill="none" stroke="#5F5E5A" stroke-width="1.4" marker-end="url(#arw)"/>')
        if elabel:
            out.append(f'<text x="{(cx_s + cx_d) / 2:.0f}" y="{midY - 4:.0f}" text-anchor="middle" '
    ...
    for nid, (x, y) in pos.items():
        ...
        out.append(f'<g><rect x="{x:.0f}" y="{y:.0f}" width="{BOX_W}" height="{BOX_H}" rx="8" '
                   f'fill="{fill}" stroke="{stroke}" stroke-width="1.2"/>')
        if len(lines) == 1:
            out.append(f'<text x="{x + BOX_W / 2:.0f}" y="{y + BOX_H / 2:.0f}" text-anchor="middle" '
                       f'dominant-baseline="central" style="font:500 13px sans-serif; fill:#2C2C2A">{esc(lines[0])}</text>')
        else:
            out.append(f'<text x="{x + BOX_W / 2:.0f}" y="{y + BOX_H / 2 - 9:.0f}" text-anchor="middle" '
                       f'style="font:500 13px sans-serif; fill:#2C2C2A">{esc(lines[0])}</text>')
            out.append(f'<text x="{x + BOX_W / 2:.0f}" y="{y + BOX_H / 2 + 11:.0f}" text-anchor="middle" '
