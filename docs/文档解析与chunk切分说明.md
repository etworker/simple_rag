...
- 编号 [1][2] 供 LLM 引用；答案中的 `[n]` 对应底部来源列表中 `idx=n` 的一条检索证据块。
- 同一文档可以包含多个 `[n]`；即使整表块与 `table_row` 块共享相同文件和位置，也不能按位置合并，否则会丢失证据。
- 前端的 `B1/B2` 仅表示文档分组，不是证据块的引用编号。
- 来源溯源：source_list 带 `idx`、text 前 200 字、文件、位置、score。
