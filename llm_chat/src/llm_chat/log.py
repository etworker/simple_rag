"""llm_chat 的 opt-in 日志配置，供 examples / 独立使用。

应用（rag_server）请在自己的入口统一配置，不要调用本函数；库内部也不会自动配置。
用法（example 脚本开头）:
    from llm_chat.log import configure_logger
    configure_logger()

file 路径默认 ``./logs/llm_chat.log``，可用环境变量 ``LLM_CHAT_LOG_FILE`` 覆盖。
"""
...
def configure_logger(level: str = "INFO") -> None:
    ...
    - file 路径：环境变量 ``LLM_CHAT_LOG_FILE``，缺省 ``./logs/llm_chat.log``
    - 同时 ``logger.enable("llm_chat")`` 打开本库日志（库默认通过 disable 关闭）
    """
    logger.remove()
